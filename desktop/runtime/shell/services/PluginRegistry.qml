...
QtObject {
  ...
  property string pluginsDir: home + "/.config/stratagem/plugins"
  ...
  function trustedCapabilities(manifest) {
    ...
    var metadata = Util.isPlainObject(manifest.stratagem) ? manifest.stratagem : null
  ...
  //   - the built-in bar option (`stratagem.bar`) is active when `bar.id` is
  //     missing or set to `stratagem.bar`.
  ...
  //     silently make `stratagem launch bar-settings` a no-op. Turning one off
  ...
  function isEnabled(id) {
    ...
    if (manifest) {
      if (Array.isArray(manifest.kinds) && manifest.kinds.indexOf("bar") !== -1) {
        ...
        if (!selectedBar) selectedBar = "stratagem.bar"
  ...
  // back, and so a plugin that is both a widget and a menu (stratagem.menu)
  ...
  function barTarget(config, placement, fallbackSection) {
    ...
    var anchors = { left: "stratagem.workspaces", center: "stratagem.weather", right: "stratagem.tray" }
  ...
  function activeCloneFor(config, sourceId) {
    for (var candidate in installedPlugins) {
      ...
      var candidateMetadata = candidateManifest && Util.isPlainObject(candidateManifest.stratagem)
        ? candidateManifest.stratagem : null
  ...
  function restoreCloneSource(config, cloneId, sourceId) {
    ...
    if (isBarOption) {
      if (sourceId === "stratagem.bar") delete config.bar.id
    ...
    shellConfigMutator(function(config) {
      ...
      if (isBarOption) {
        if (value) {
          ...
          if (clonedFrom && clonedFrom !== "stratagem.bar") config.bar.id = clonedFrom
  ...
  function parseScanOutput(text) {
    ...
    // `stratagem.*` namespace is reserved for built-ins, including bar widgets
    // registered outside the manifest-based plugin registry.
    for (var tk in thirdParty) {
      if (firstParty[tk] || String(tk).indexOf("stratagem.") === 0) {
        console.warn("PluginRegistry: plugin " + tk
          + " rejected: id is reserved for first-party STRATAGEM DARK plugins")
        continue
      }
  ...
  function rescan() {
    ...
    // Third-party plugins stay at the top level of ~/.config/stratagem/plugins.
    var script = ""
