...
QtObject {
  ...
  // component follows the fontconfig alias `stratagem-font-set` writes.
  ...
  // alias path keeps working when the user runs `stratagem font set`), but
  ...
  // STRATAGEM_DARK_MENU_FONT override for users who want a different family on the
  ...
  readonly property string menuFontFamily: {
    var override = Quickshell.env("STRATAGEM_DARK_MENU_FONT")
  ...
  // Resolve the fontconfig alias to a concrete family name. `stratagem font
  ...
  // `stratagem toggle window-gaps` creates/removes this flag file. Hyprland
  ...
  property FileView windowNoGapsToggle: FileView {
    path: Quickshell.env("HOME") + "/.local/state/stratagem/toggles/hypr/window-no-gaps.lua"
    watchChanges: true
    printErrors: false
