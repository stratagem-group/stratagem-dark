...
QtObject {
  ...
  readonly property string currentThemePath: stateHome + "/stratagem/current/theme"
  ...
  // Style. User keys win, so a machine-level `~/.config/stratagem/shell.toml`
  ...
  // is where `stratagem display text size` writes `[font] base-size`. Watched so the
  // CLI takes effect live without restarting the shell; absent by default.
  property FileView userShellFile: FileView {
    id: userShellFile
    path: root.home + "/.config/stratagem/shell.toml"
    watchChanges: true
    printErrors: false
