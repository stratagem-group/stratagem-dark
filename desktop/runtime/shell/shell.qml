...
ShellRoot {
  ...
  // The stratagem-shell host is the long-running entry point. Plugins live in
  // sibling directories under plugins/. STRATAGEM_DARK_PATH is provided by the uwsm
  // session environment and is the single source of truth for this checkout.
  property string stratagemPath: Quickshell.env("STRATAGEM_DARK_PATH")
  readonly property string shellPath: stratagemPath + "/shell"
  readonly property string firstPartyPluginsDir: shellPath + "/plugins"
  readonly property string defaultsPath: stratagemPath + "/config/stratagem/shell.json"
  readonly property string userConfigPath: home + "/.config/stratagem/shell.json"
  ...
  readonly property var builtinShellConfig: ({
    ...
    bar: {
      position: "top",
      transparent: false,
      centerAnchor: "stratagem.clock",
      layout: {
        left: [{ id: "stratagem.menu" }, { id: "stratagem.workspaces" }],
        center: [{ id: "stratagem.clock", format: "dddd HH:mm" }],
        right: [{ id: "stratagem.audio" }]
      }
    },
    plugins: []
  })
  ...
  Component.onCompleted: {
    console.log("stratagem-shell paths",
      "stratagemPath=" + shell.stratagemPath,
  ...
  readonly property string defaultBarId: "stratagem.bar"
  ...
  function configureBar(target, manifest) {
    if (!target) return
    if ("stratagemPath" in target) target.stratagemPath = shell.stratagemPath
  ...
  function publicIdleConfigFor(manifest) {
    var metadata = manifest && Util.isPlainObject(manifest.stratagem) ? manifest.stratagem : null
    if (!metadata || String(metadata.clonedFrom || "") !== "stratagem.idle") return ({})
  ...
  function pluginCloneMaySummon(manifest, requestedId) {
    ...
    var allowed = {
      "stratagem.audio": ["stratagem.osd"],
      "stratagem.media": ["stratagem.osd"],
      "stratagem.monitor": ["stratagem.osd"],
      "stratagem.network": ["stratagem.speedtest", "stratagem.wifiqr"]
    }
  ...
  function pluginFirstPartyServiceFor(cacheKey, pluginId, requestedId) {
    ...
    var allowed = ["stratagem.idle", "stratagem.media", "stratagem.nightlight", "stratagem.notifications"]
  ...
  function createScopedPluginShell(manifest, cacheKey, allowOwnService, barCapabilities) {
    ...
    if (barCapabilities) {
      var serviceIds = ["stratagem.idle", "stratagem.media", "stratagem.nightlight", "stratagem.notifications"]
  ...
  function ensureService(pluginId) {
    ...
    function finalize() {
      ...
      if ("stratagemPath" in inst) inst.stratagemPath = shell.stratagemPath
  ...
  // Destroying stratagem.lock drops the ext-session-lock client while Hyprland
  ...
  function isBarWidgetPanelPlugin(pluginId) {
    ...
    // panel loader (e.g. stratagem.menu); let that path handle them.
  ...
  Instantiator {
    ...
    delegate: QtObject {
      ...
      property Loader panelLoader: Loader {
        ...
        onLoaded: {
          if (!item) return
          if ("stratagemPath" in item) item.stratagemPath = shell.stratagemPath
  ...
  function imagePickerItem() {
    var loader = panelLoaders["stratagem.image-picker"]
  ...
  IpcHandler {
    ...
    function open(imageDirs: string,
      ...
      return shell.summon("stratagem.image-picker", payload) ? "ok" : "unknown"
    }
    ...
    function cancel(doneFile: string): string {
      ...
      if (picker && typeof picker.closeSelector === "function") {
        ...
        shell.hide("stratagem.image-picker")
      }
      return "ok"
    }
  ...
  IpcHandler {
    ...
    function listPlugins(): string {
      ...
      for (var id in plugins) {
        ...
        var metadata = plugins[id].stratagem
        var clonedFrom = Util.isPlainObject(metadata) ? String(metadata.clonedFrom || "") : ""
        out.push({
          id: id,
          name: plugins[id].name,
          kinds: kinds,
          // What `stratagem plugin enable/disable` toggles: for a widget that is
