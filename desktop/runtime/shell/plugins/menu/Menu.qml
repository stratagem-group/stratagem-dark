...
Item {
  ...
  // Injected by stratagem-shell when this plugin is summoned.
  property string stratagemPath: Quickshell.env("STRATAGEM_DARK_PATH")
  property var shell: null
  property var manifest: null

  // Plugin lifecycle hooks. The host calls open(payloadJson) after
  // `stratagem-shell shell summon stratagem.menu ...` and close() when hidden.
  ...
  property string defaultMenuPath: stratagemPath + "/default/stratagem/stratagem-menu.jsonc"
  property string userMenuPath: Quickshell.env("HOME") + "/.config/stratagem/extensions/stratagem-menu.jsonc"
  ...
  readonly property var providers: ({
    "fonts": {
      script: "current=$(stratagem-font-current 2>/dev/null); stratagem-font-list 2>/dev/null | while read -r f; do [[ -z $f ]] && continue; printf '%s\\t%s\\t%s\\n' \"$f\" \"$f\" \"$current\"; done",
      icon: "",
      volatile: true,
      actionFor: function(value) { return "stratagem-font-set " + Util.shellQuote(value) }
    },
    "power-profiles": {
      script: "current=$(powerprofilesctl get 2>/dev/null); stratagem-powerprofiles-list 2>/dev/null | while read -r p; do [[ -z $p ]] && continue; printf '%s\\t%s\\t%s\\n' \"$p\" \"$p\" \"$current\"; done",
      icon: "\udb81\udc0b",
      actionFor: function(value) { return "stratagem-powerprofiles-set autodetect " + Util.shellQuote(value) }
    }
  })
  ...
  // `stratagem-shell shell summon stratagem.menu '{"menu":"system"}'`.
  ...
  function openRoute(initialMenu) {
    ...
    // a leaf, e.g. `stratagem menu summon screenrecord-stop`), run it directly
  ...
  // user extension at ~/.config/stratagem/extensions/stratagem-menu.jsonc) take
  ...
  PanelWindow {
    ...
    color: "transparent"
    WlrLayershell.namespace: "stratagem-menu"
