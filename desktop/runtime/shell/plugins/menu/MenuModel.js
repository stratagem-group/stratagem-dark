...
// against `stratagem-default-browser` -- so the batch runs it once and the rows
...
var GUARD_READERS = [
  "stratagem-channel-current",
  "stratagem-default-agent",
  "stratagem-default-browser",
  "stratagem-default-editor",
  "stratagem-default-terminal",
  "stratagem-dns"
]
...
function guardHelpers() {
  return 'declare -A __stratagem_pkgs=()\n'
    + 'mapfile -t __stratagem_pkg_names < <({ pacman -Qq; LC_ALL=C pacman -Qi'
    + " | awk '/^[A-Za-z]/ { provides = ($0 ~ /^Provides/); sub(/^[^:]*: /, \"\") }"
    + ' provides && $0 != "None" { n = split($0, p, " ");'
    + ' for (i = 1; i <= n; i++) { sub(/[<>=].*/, "", p[i]); print p[i] } }\'; } 2>/dev/null)\n'
    + 'for __stratagem_pkg in "${__stratagem_pkg_names[@]}"; do __stratagem_pkgs[$__stratagem_pkg]=1; done\n'
    + '__stratagem_pkg_has() { [[ -n ${__stratagem_pkgs[$1]-} ]] && return 0; '
    + '[[ $1 == *[\\<\\>=]* ]] && { pacman -Q "$1" &>/dev/null; return; }; return 1; }\n'
    + 'stratagem-pkg-present() { local p; for p in "$@"; do __stratagem_pkg_has "$p" || return 1; done; return 0; }\n'
    + 'stratagem-pkg-missing() { local p; for p in "$@"; do __stratagem_pkg_has "$p" || return 0; done; return 1; }\n'
    + 'stratagem-cmd-present() { local c; for c in "$@"; do command -v "$c" &>/dev/null || return 1; done; return 0; }\n'
    + 'stratagem-cmd-missing() { local c; for c in "$@"; do command -v "$c" &>/dev/null || return 0; done; return 1; }\n'
}
...
function guardPrelude(guards) {
  ...
  for (var i = 0; i < GUARD_READERS.length; i++) {
    ...
    prelude += "__stratagem_read_" + i + "=$(" + GUARD_READERS[i] + " 2>/dev/null) || :\n"
  }
...
function guardReaderSlot(index) {
  return "${__stratagem_read_" + index + "}"
}
