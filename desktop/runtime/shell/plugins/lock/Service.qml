...
Item {
  ...
  property string stratagemPath: ""

  readonly property string home: Quickshell.env("HOME")
  readonly property string stateHome: home + "/.local/state"
  readonly property string userName: Quickshell.env("USER") || Quickshell.env("LOGNAME")
  readonly property string currentBackgroundLink: stateHome + "/stratagem/current/background"
  ...
  readonly property var batteryService: shell && shell.services ? shell.firstPartyServiceFor("stratagem.battery") : null
  ...
  function logEvent(event) {
    ...
    console.log("stratagem lock " + lastEventAt + " " + event)
  }
  ...
  PanelWindow {
    ...
    color: "transparent"
    WlrLayershell.namespace: "stratagem-lock-preview"
  ...
  PamContext {
    id: passwordPam
    config: "stratagem-lock-password"
    user: root.userName
  ...
  PamContext {
    id: fingerprintPam
    config: "stratagem-lock-fingerprint"
    user: root.userName
  ...
  Process {
    ...
    command: ["bash", Quickshell.env("STRATAGEM_DARK_PATH") + "/shell/plugins/lock/poster.sh", sourcePath]
  ...
  Process {
    ...
    command: ["bash", "-c", "if [[ -f /etc/pam.d/stratagem-lock-fingerprint ]] && command -v fprintd-list >/dev/null 2>&1 && fprintd-list \"$USER\" 2>/dev/null | grep -qi finger; then echo yes; else echo no; fi"]
  ...
  Process {
    ...
    command: ["bash", "-c", "stratagem-hyprland-session-locked"]
    onExited: function(exitCode) {
  ...
  Process {
    id: wakeProcess
    command: ["bash", "-c", "stratagem-system-wake"]
  }

  Process {
    id: blankProcess
    command: ["bash", "-c", "stratagem-brightness-keyboard off; stratagem-brightness-display off"]
  }
  ...
  FileView {
    path: "/etc/pam.d/stratagem-lock-password"
    watchChanges: true
    printErrors: false
