...
Item {
  ...
  readonly property string stayAwakeStateDir: home + "/.local/state/stratagem/indicators"
  ...
  readonly property string screensaverClass: "org.stratagem.screensaver"
  ...
  function logEvent(event, details) {
    ...
    console.log("stratagem idle " + root.lastEventAt + " " + root.lastEvent)
  }
  ...
  function launchScreensaver() {
    ...
    runProcess(screensaverProcess, "screensaver", "[[ $(stratagem-shell lock isLocked 2>/dev/null) == \"true\" ]] || stratagem-launch-screensaver")
  }
  ...
  function lockSystem(reason) {
    ...
    runProcess(lockProcess, "lock", "stratagem-system-lock")
  }
  ...
  function cancelIdleCycle(reason) {
    ...
    if (root.idledThisCycle) runProcess(wakeProcess, "wake", "stratagem-system-wake")
  ...
  function persistStayAwake(value) {
    var command = value
      ? "mkdir -p \"$HOME/.local/state/stratagem/indicators\" && touch \"$HOME/.local/state/stratagem/indicators/stay-awake\""
      : "rm -f \"$HOME/.local/state/stratagem/indicators/stay-awake\""
  ...
  Process {
    ...
    command: ["bash", "-c", "mkdir -p \"$HOME/.local/state/stratagem/indicators\"; if [[ -f $HOME/.local/state/stratagem/indicators/stay-awake ]]; then echo yes; else echo no; fi"]
    stdout: SplitParser {
