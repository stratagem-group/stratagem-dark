...
Item {
  ...
  // The stratagem-shell host injects stratagemPath from STRATAGEM_DARK_PATH.
  ...
  property string stratagemConfigDir: home + "/.config/stratagem"
  ...
  // to "monospace"), so changing the system font (via `stratagem-font-set`)
  ...
  // Resolve the live bar-widget instance for a plugin id (e.g. "stratagem.bluetooth").
  ...
  function customModuleSource(entry) {
    var source = BarModel.customModulePath(entry, home, stratagemConfigDir)
  ...
  FileView {
    path: root.stateHome + "/stratagem/current"
    watchChanges: true
    printErrors: false
  ...
  // and the flag is created/removed by `stratagem-toggle-bar`.
  Process {
    id: barHiddenProbe
    running: true
    command: ["bash", "-c", "[[ -f $HOME/.local/state/stratagem/toggles/bar-off ]] && echo yes || echo no"]
    stdout: SplitParser { onRead: function(line) { root.barHidden = String(line).trim() === "yes" } }
  }
  FileView {
    path: root.home + "/.local/state/stratagem/toggles"
    watchChanges: true
    printErrors: false
  ...
  // shell restarts. `stratagem-toggle-bar` nudges this after flipping the flag
  // so the probe re-reads it even when the watch has gone quiet.
  IpcHandler {
    target: "stratagem.bar"
  ...
  component BarPanel: PanelWindow {
    ...
    WlrLayershell.namespace: "stratagem-bar"
  ...
  component DragGhostPanel: PanelWindow {
    ...
    WlrLayershell.namespace: "stratagem-bar-drag-ghost"
  ...
  component BarMoveGhostPanel: PanelWindow {
    ...
    color: "transparent"
    exclusionMode: ExclusionMode.Ignore
    WlrLayershell.namespace: "stratagem-bar-move-ghost"
    WlrLayershell.layer: WlrLayer.Overlay
    WlrLayershell.keyboardFocus: WlrKeyboardFocus.None
