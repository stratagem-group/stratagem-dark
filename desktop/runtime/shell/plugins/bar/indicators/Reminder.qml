...
BarIndicator {
  ...
  function openReminderFlow() {
    Quickshell.execDetached(["stratagem-reminder", "-i"])
  }
  ...
  Process {
    id: jsonProc
    command: ["stratagem-reminder", "show", "--json"]
  ...
  onPressed: function() {
    if (root.reminderCount > 0) Quickshell.execDetached(["stratagem-reminder", "show"])
