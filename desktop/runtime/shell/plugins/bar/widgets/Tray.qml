...
BarWidget {
  ...
  // UseQApplication` - stratagem's shell.qml does not, so every submenu click was
  ...
  function ownedByStratagemDark(item) {
    var layout = root.bar && root.bar.layoutConfig ? root.bar.layoutConfig : null
    return TrayModel.ownedByStratagemDark(item, layout)
  }
  ...
  function bucket(category) {
    ...
    for (var i = 0; i < values.length; i++) {
      ...
      if (ownedByStratagemDark(item)) continue
      if (category === "all") {
        result.push(item)
        continue
      }
  ...
  function persistTrayState(pinned, hidden) {
    ...
    var id = root.moduleName || "stratagem.tray"
