...
Item {
  ...
  Process {
    id: bgSwitchProc
    command: ["bash", "-c", "background=$(stratagem-theme-bg-switcher); [[ -n $background ]] && stratagem-theme-bg-set \"$background\""]
    onExited: root.refreshBackground()
  }

  Process {
    id: themeSwitchProc
    command: ["bash", "-c", "theme=$(stratagem-theme-switcher); [[ -n $theme ]] && stratagem-theme-set \"$theme\" >/dev/null 2>&1 &"]
    onExited: root.refreshBackground()
  }
  ...
  Variants {
    ...
    PanelWindow {
      ...
      // leaving a black desktop until stratagem-shell is restarted. A still
      ...
      WlrLayershell.namespace: "stratagem-background"
