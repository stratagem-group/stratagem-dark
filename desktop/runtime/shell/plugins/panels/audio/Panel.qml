...
Panel {
  id: root
  moduleName: "stratagem.audio"
  ipcTarget: "stratagem.audio"

  readonly property var sink: Pipewire.defaultAudioSink
  readonly property var source: Pipewire.defaultAudioSource
  readonly property var nodes: Pipewire.nodes ? Pipewire.nodes.values : []
  readonly property var mprisPlayers: Mpris.players ? Mpris.players.values : []
  readonly property var mediaService: bar?.shell?.firstPartyServiceFor("stratagem.media")
  ...
  readonly property var candidateStreams: {
    ...
    for (var i = 0; i < nodes.length; i++) {
      ...
      if (String(n.name || "").indexOf("stratagem_speaker_tuning") === 0) continue
      list.push(n)
    }
    return list
  }
  ...
  //
  // stratagem-audio-output-sink resolves the *current* default output through any
  ...
  function setDefaultSource(node) {
    ...
    if (node.id !== undefined && node.name) {
      Quickshell.execDetached([
        "stratagem-audio-input-set-default",
        String(node.id),
        String(node.name)
      ])
    }
  ...
  Process {
    ...
    command: ["stratagem-audio-sink-availability"]
  ...
  Process {
    id: volumeSinkProc
    command: ["stratagem-audio-output-sink"]
    stdout: StdioCollector {
      waitForEnd: true
