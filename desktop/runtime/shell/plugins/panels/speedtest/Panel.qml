...
// stratagem-network-status when the caller doesn't know it.
Item {
  id: root
  ...
  function dismiss() {
    if (root.shell && typeof root.shell.hide === "function")
      root.shell.hide((root.manifest && root.manifest.id) || "stratagem.speedtest")
    else close()
  }
  ...
  function startPhase(nextPhase) {
    ...
    phase = nextPhase
    stderrText = ""
    speedTestProc.command = ["stratagem-network-speedtest", nextPhase]
  ...
  Process {
    id: statusProc
    command: ["stratagem-network-status"]
  ...
  SpeedTestOverlay {
    ...
    layerNamespace: "stratagem-network-speedtest"
