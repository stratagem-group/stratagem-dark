...
Panel {
  id: root
  moduleName: "stratagem.network"
  ipcTarget: "stratagem.network"
  ...
  // `stratagem-network-status --verbose` samples (~1.5s apart via detailsPoll).
  ...
  // Wi-Fi band state from `stratagem-network-band`. `bandCurrent` is the band
  ...
  function openCaptivePortal() {
    ...
    Quickshell.execDetached(["stratagem-launch-browser", Model.captivePortalUrl])
    close()
  }
  ...
  // The share card is its own panel plugin (stratagem.wifiqr) so a replacement
  ...
  function summonWifiQr(forceDetect) {
    ...
    bar.shell.summon("stratagem.wifiqr", JSON.stringify(payload))
  }
  ...
  function refresh(scanWifi) {
    ...
    if (!bandProc.running) {
      bandProc.command = ["stratagem-network-band"]
  ...
  function setBand(band) {
    ...
    actionProc.command = ["stratagem-network-band", band]
    actionProc.running = true
  }

  // The speed test is its own panel plugin (stratagem.speedtest) so a
  ...
  function summonSpeedTest() {
    ...
    bar.shell.summon("stratagem.speedtest", connection ? JSON.stringify({ connection: connection }) : "{}")
  }

  function dnsCommand(provider) {
    var command = "stratagem-dns"
  ...
  Process {
    id: detailsProc
    command: ["stratagem-network-status", "--verbose"]
