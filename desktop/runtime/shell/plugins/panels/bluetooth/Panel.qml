...
Panel {
  id: root
  moduleName: "stratagem.bluetooth"
  ipcTarget: "stratagem.bluetooth"
  // manageIpc: false so this panel can own the single IpcHandler the target
  // permits — needed for the toggleBluetooth method below.
  manageIpc: false

  // Address -> "connecting" | "disconnecting" | "forgetting".
  // The actual Bluetooth sequencing lives in bin/stratagem-bluetooth-device;
  ...
  function setDefaultAudioSink(sink) {
    ...
    if (sink.id !== undefined && sink.name) {
      Quickshell.execDetached([
        "stratagem-audio-output-set-default",
        String(sink.id),
        String(sink.name)
      ])
    }
  ...
  function deviceCommand(action, address) {
    return ["stratagem-bluetooth-device", action, address]
  }
  ...
  // disconnects, then removes the BlueZ pairing record via stratagem-bluetooth-device.
  ...
  // the adapter came back on at the next boot. stratagem-bluetooth-power moves the
  ...
  function toggleBluetooth() {
    if (!adapter) return
    Quickshell.execDetached(["stratagem-bluetooth-power", adapter.enabled ? "off" : "on"])
  }

  IpcHandler {
    target: "stratagem.bluetooth"
