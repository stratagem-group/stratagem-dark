...
// stratagem-network-qr, which emits the interface, security, and SSID it
...
Item {
  ...
  function dismiss() {
    if (root.shell && typeof root.shell.hide === "function")
      root.shell.hide((root.manifest && root.manifest.id) || "stratagem.wifiqr")
    else close()
  }
  ...
  function generate(requestedIface) {
    ...
    qrProc.command = requestedIface
      ? ["stratagem-network-qr", "--meta", requestedIface]
      : ["stratagem-network-qr", "--meta"]
    qrProc.running = true
  }
  ...
  function togglePassword() {
    ...
    pwProc.command = ["stratagem-network-password", iface]
    pwProc.running = true
  }
  ...
  PanelWindow {
    ...
    WlrLayershell.namespace: "stratagem-network-qr"
