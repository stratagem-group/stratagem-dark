...
Panel {
  id: root
  moduleName: "stratagem.monitor"
  ipcTarget: "stratagem.monitor"
  manageIpc: false
  ...
  // these stops; the CLI (stratagem-display-text-size) accepts any integer in range.
  ...
  function setBrightness(value) {
    ...
    setBrightnessProc.command = ["stratagem-brightness-display", "--no-osd", "--monitor", root.focusedMonitor, percent + "%"]
  ...
  function showBrightnessOsd(percent) {
    ...
    bar.shell.summon("stratagem.osd", JSON.stringify({
      icon: "brightness",
      value: percent
    }))
  }
  ...
  function setScale(scale) {
    actionProc.command = ["bash", "-c", "stratagem-hyprland-monitor-scaling " + scale]
    if (!actionProc.running) actionProc.running = true
  }
  ...
  function setTextSize(px) {
    textScaleProc.command = ["stratagem-display-text-size", String(px)]
  ...
  Process {
    id: stateProc
    command: ["stratagem-monitor-state"]
  ...
  Process {
    ...
    // `stratagem-brightness-display` races the hardware/driver and can
