...
Panel {
  id: root
  moduleName: "stratagem.weather"
  ipcTarget: "stratagem.weather"
  manageIpc: false
  ...
  // stratagem-weather-location). The query is the wttr.in path segment
  ...
  property FileView locationFile: FileView {
    path: Quickshell.env("HOME") + "/.local/state/stratagem/settings/weather.json"
    watchChanges: true
    printErrors: false
  ...
  function persistLocation(name, latitude, longitude) {
    if (name && latitude !== null && longitude !== null)
      locationSaveProc.command = ["stratagem-weather-location", "--set", name, latitude + "," + longitude]
    else if (name)
      locationSaveProc.command = ["stratagem-weather-location", "--set", name]
    else
      locationSaveProc.command = ["stratagem-weather-location", "--clear"]
  ...
  // Mirrors stratagem-weather-icon's wttr.in code → nerd-font glyph mapping.
