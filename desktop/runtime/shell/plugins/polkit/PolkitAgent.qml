...
Item {
  ...
  Process {
    id: laptopClosedProc
    command: ["bash", "-c", "stratagem-hw-laptop-closed && echo closed || echo open"]
  ...
  PolkitAgent {
    id: polkitAgent
    path: "/org/stratagem/PolkitAgent"
    ...
    onIsRegisteredChanged: {
      if (isRegistered) console.log("stratagem polkit agent registered")
      else console.warn("stratagem polkit agent is not registered; another agent may be running")
    }
  }
  ...
  PanelWindow {
    ...
    color: "transparent"
    WlrLayershell.namespace: "stratagem-polkit"
