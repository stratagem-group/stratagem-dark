...
function glyphFromHints(hints) {
  return stringHint(hints, "stratagem-glyph")
}

// The click action: a JSON argv string from stratagem-notification-send
...
function execArgvFromHints(hints) {
  return stringHint(hints, "stratagem-exec-argv")
}

// Validate a persisted stratagem-exec-argv into a runnable argv, or null. This is
...
// ~/.local/state/stratagem/notifications/ so toasts survive shell restarts
// (e.g. the restart `stratagem-update` performs). The file exists exactly as
...
// senders (all STRATAGEM DARK web apps) delete their scoped /tmp files on close,
