// Notification service for the stratagem shell.
...
Item {
  ...
  // Injected by stratagem-shell (the first-party service loader).
  ...
  readonly property string stateDir: home + "/.local/state/stratagem/"
  ...
  // Toasts are fixed to the top-right corner. They only clear the stratagem bar
  ...
  PersistentProperties {
    id: persisted
    reloadableId: "stratagem-notifications"
  ...
  // and rare.
  //   - stratagem-action: a user-action confirmation toast ("Theme changed",
  ...
  //     Trusted because it's almost always stratagem or system shell scripts —
  ...
  //   - app_name is "stratagem-action" (STRATAGEM DARK's own user-action toasts —
  ...
  // Run the popup's click action, then dismiss. STRATAGEM DARK's own toasts carry the
  ...
  function focusApp(entry) {
    ...
    focusAppProc.command = [
      service.stratagemPath + "/bin/stratagem-hyprland-focus-app",
      String(entry.app)
    ]
  ...
  // toasts survive shell restarts (notably the restart `stratagem-update`
  ...
  Variants {
    ...
    PanelWindow {
      ...
      WlrLayershell.namespace: "stratagem-notifications"
