...
// stratagem-agent-usage-update, which writes one JSON record per agent into
...
Item {
  ...
  readonly property string usageDir: (Quickshell.env("XDG_STATE_HOME") || home + "/.local/state") + "/stratagem/agents/usage"
  ...
  function updateCommand(kind, agentIds) {
    var command = ["stratagem-agent-usage-update"]
  ...
  readonly property string syncSnapshotPath: syncConfigured() ? syncEffectiveDir + "/" + syncEffectiveFileName : home + "/.cache/stratagem/agents-disabled.json"
  ...
  // Snapshots keep the field names older STRATAGEM DARK versions wrote, so a fleet
