...
local function command_from(value, description)
  ...
  if value.stratagem then
    return "stratagem-launch-" .. value.stratagem
  ...
  elseif value.tui then
    if value.focus then
      return "stratagem-launch-or-focus-tui " .. shell_quote(value.tui)
    else
      return "stratagem-launch-tui " .. shell_quote(value.tui)
    end
  end
...
function o.preinstalled_bindings_enabled()
  if _G.stratagem_preinstalled_bindings ~= nil then
    return _G.stratagem_preinstalled_bindings == true
  end

  return not file_exists((os.getenv("HOME") or "") .. "/.local/state/stratagem/preinstalls-removed")
end
...
function o.launch_webapp(url)
  return "stratagem-launch-webapp " .. shell_quote(url)
end

function o.launch_webapp_sole(name, url)
  return "stratagem-launch-or-focus-webapp " .. shell_quote(name) .. " " .. shell_quote(url)
end

function o.launch_sole(match, command)
  return "stratagem-launch-or-focus " .. shell_quote(match) .. " " .. shell_quote(o.launch(command))
end

function o.bind_toggle(keys, description, toggle, options)
  o.bind(keys, description, "stratagem-toggle-" .. toggle, options)
end

function o.notify(message)
  return "stratagem-notification-send -u low " .. shell_quote(message)
end
