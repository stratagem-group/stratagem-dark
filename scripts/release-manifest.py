...
manifest={'schema_version':1,'product':'STRATAGEM DARK','version':'0.2.0-alpha4','source_commit':commit,'architecture':'x86_64','arch_snapshot':(bundle/'snapshot.txt').read_text().strip(),'builder_image':(bundle/'builder-image.txt').read_text().strip(),'repository_databases':{p.name:digest(p) for p in sorted((db/'sync').glob('*.db'))},'packages':packages,'files':files,'release_class':'testing'}
